function applySimpleStartup(html) {
  ...
  <div class="location-intro-core">
    <div class="location-intro-brand" aria-label="Frenano">
      <img class="location-intro-logo" src="/images/frenano-startup-logo-512.png?v=20260911-frenano-location-v5" alt="">
...
<style id="simple-startup-v4" data-flow="v5-smooth">
  ...
  .location-intro-detail { width:100%; display:flex; flex-direction:column; align-items:center; transition:opacity .28s ease,transform .34s ease; }
  ...
  .location-enable-button { width:100%; margin:22px 0 0; min-height:56px; background:linear-gradient(180deg,#279cff,#0b82ee); color:#fff; box-shadow:0 10px 28px rgba(0,92,200,.28); transition:opacity .24s ease,transform .24s ease; }
  .location-intro-values { margin-top:auto; max-width:410px; padding-top:18px; font-size:10px; line-height:1.55; font-weight:650; letter-spacing:.055em; color:rgba(255,255,255,.52); transition:opacity .24s ease; }
  ...
  /* Returning web visits still need a deliberate tap so the browser can keep the screen awake. */
  .frenano-location-intro.returning .location-intro-detail,
  .frenano-location-intro.returning .location-intro-values { display:none; }
  .frenano-location-intro.returning .location-intro-core { justify-content:center; padding-top:max(24px,env(safe-area-inset-top)); padding-bottom:max(24px,env(safe-area-inset-bottom)); }
  .frenano-location-intro.returning .location-intro-logo { width:112px; height:112px; border-radius:28px; }
  .frenano-location-intro.returning .location-intro-name { margin-top:18px; font-size:32px; }
  .frenano-location-intro.returning .location-intro-tagline { margin-top:10px; font-size:15px; }
  .frenano-location-intro.returning .location-enable-button { width:min(310px,100%); margin-top:30px; background:#fff; color:#000; box-shadow:none; }
  .frenano-location-intro.returning.returning-native .location-enable-button { display:none; }

  /* One restrained transition: content disappears, brand stays as the visual anchor, then the overlay itself fades away. */
  .frenano-location-intro.departing .location-intro-detail { opacity:0; transform:translateY(-8px); }
  .frenano-location-intro.departing .location-enable-button { opacity:0; transform:scale(.985); pointer-events:none; }
  .frenano-location-intro.departing .location-intro-values { opacity:0; }
  .frenano-location-intro.departing .location-intro-brand { transform:translateY(3px) scale(1.015); }
  .frenano-location-intro.departing .location-intro-shade { opacity:.88; }
...
(() => {
  ...
  function hideLaunch() {
    ...
    launch?.classList.remove('intro-visible');
  ...
  function beginExit() {
    ...
    exitTimer = window.setTimeout(hideLaunch, 520);
  }
  ...
  if (seen) {
    ...
    requestAnimationFrame(() => launch?.classList.add('intro-visible'));

    // Native iOS can start without another confirmation. Web deliberately keeps
    // the single Let’s go! tap so the existing start handler can request wake lock.
    if (native && sourceButton && !sourceButton.disabled) {
      window.setTimeout(() => sourceButton.click(), 90);
    }
  } else {
    launch?.setAttribute('aria-hidden','false');
    requestAnimationFrame(() => launch?.classList.add('intro-visible'));
