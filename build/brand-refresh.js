function applyBrandRefresh(html) {
  const brandRevision = "20260909-frenano-v3";
  const startupMark = `/images/frenano-startup-logo-512.png?v=${brandRevision}`;
  const runningMark = `/images/frenano-running-logo-256.png?v=${brandRevision}`;
  const websiteMark = `/images/frenano-website-icon-512.png?v=${brandRevision}`;
  const launchBefore = `    <div class="launch-ring">\n      <div class="launch-mark">Super Simple Speedo</div>\n    </div>`;
  const launchAfter = `    <div class="launch-brand" aria-label="Frenano">\n      <div class="launch-brand-icon"><img class="launch-brand-mark" src="${startupMark}" alt=""></div>\n      <div class="launch-brand-name"><strong>Frenano</strong></div>\n      <div class="launch-brand-tagline">GPS speedometer, simply done.</div>\n    </div>`;
  const runningBefore = `    <div class="app-title">Super Simple Speedo</div>`;
  const runningAfter = `    <div class="app-title app-brand-lockup" aria-label="Frenano"><img class="app-brand-mark" src="${runningMark}" alt=""><span class="app-brand-name"><strong>Frenano</strong></span></div>`;
  ...
  const newMetadata = '<title>Frenano — GPS speedometer, simply done.</title>\n  <meta name="description" content="Frenano is a simple GPS speedometer with automatic local speed-limit information where available. Completely free. No account, subscription or advertisements. Privacy focused and made in Switzerland.">\n  <meta name="apple-mobile-web-app-title" content="Frenano">';
