...
(() => {
  ...
  function updateUnitsUi() {
    ...
    const kmhButton = document.getElementById('unitKmhButton');
    const mphButton = document.getElementById('unitMphButton');
    kmhButton?.classList.toggle('active', !mph);
    mphButton?.classList.toggle('active', mph);
    ...
    document.querySelectorAll('#limitGrid .limit-choice[data-limit]').forEach(button => {
      const raw = Number(button.dataset.limit);
      if (Number.isFinite(raw)) button.textContent = String(asDisplay(raw));
    });
  ...
  function installUnitsSetting(displaySection) {
    ...
    if (!body || body.querySelector('#unitKmhButton')) return;
    const appearance = body.querySelector('.setting');
    if (!appearance) return;

    const units = document.createElement('div');
    units.className = 'setting';
    ...
    appearance.insertAdjacentElement('afterend', units);

    document.getElementById('unitKmhButton')?.addEventListener('click', () => {
      localStorage.setItem(unitKey, 'kmh');
      updateUnitsUi();
    });
    document.getElementById('unitMphButton')?.addEventListener('click', () => {
      localStorage.setItem(unitKey, 'mph');
      updateUnitsUi();
    });
  }

  function compactVisibleElements(displaySection) {
    const body = displaySection?.querySelector('.settings-section-body');
    if (!body) return;
    const visibleTitle = [...body.querySelectorAll('.setting-title')].find(el => el.textContent.trim() === 'Visible elements');
    const setting = visibleTitle?.closest('.setting');
    if (!setting) return;
    const lists = setting.querySelectorAll('.choice-list');
    lists[0]?.classList.add('compact-choice-list');
    lists[1]?.classList.add('compact-choice-list');
  }
  ...
  async function installLocationSetting(privacySection) {
    const body = privacySection?.querySelector('.settings-section-body');
    if (!body || body.querySelector('.location-setting')) return;

    const location = document.createElement('div');
    location.className = 'setting location-setting';
    location.innerHTML = '<div class="location-row"><div><div class="setting-title">Location access</div><div class="location-state" id="locationPermissionState"><span class="location-dot"></span><span>Checking…</span></div></div><button class="manage-location" id="manageLocationButton" type="button">Manage</button></div><div class="location-help" id="locationPermissionHelp"></div>';
    body.prepend(location);

    const render = async () => {
      const result = await permissionState();
      const status = document.getElementById('locationPermissionState');
      if (!status) return;
      ...
      if (result.state === 'granted') {
        ...
        if (label) label.textContent = 'Location blocked';
      } else if (result.state === 'prompt') {
        if (label) label.textContent = 'Not yet shared';
      } else {
        if (label) label.textContent = 'Managed by this device';
      }
      if (result.status) result.status.onchange = render;
    };

    document.getElementById('manageLocationButton')?.addEventListener('click', async () => {
      ...
      help.textContent = ios
        ? 'To change location access, open iPhone Settings and choose Frenano (or Safari) → Location.'
        : 'To change location access, use this browser’s site permissions for Location.';
  ...
  function decorateSettings() {
    ...
    if (!modal || modal.dataset.redesignedV2 === 'true') return;
    modal.dataset.redesignedV2 = 'true';

    const display = modal.querySelector('[data-settings-section="display"]');
    const about = modal.querySelector('[data-settings-section="about"]');
    const privacy = modal.querySelector('[data-settings-section="privacy"]');

    [display, about, privacy].forEach(section => {
      if (!section) return;
      section.classList.add('open');
      section.querySelector('.settings-section-header')?.setAttribute('aria-expanded','true');
    });

    const displayTitle = display?.querySelector('.settings-section-title');
    if (displayTitle) displayTitle.textContent = 'Appearance & Display';

    installUnitsSetting(display);
    compactVisibleElements(display);
    installLocationSetting(privacy);
    updateUnitsUi();
    ...
    if (sheet && !sheet.querySelector('.settings-redesign-note')) {
      ...
      note.textContent='Frenano · Made in Switzerland';
