...
<style id="settings-polish-v1">
  ...
  #settingsModal .location-permission-row {
    display:grid;
    grid-template-columns:minmax(0,1fr) auto;
    gap:12px;
    align-items:center;
  }
  #settingsModal .location-permission-status {
    display:flex;
    align-items:center;
    gap:7px;
    margin-top:6px;
  ...
  #settingsModal .location-permission-action {
    min-width:96px;
    min-height:40px;
    padding:0 13px;
    border:0;
  ...
  #settingsModal .location-permission-help {
    display:none;
    margin-top:10px;
    font-size:12px;
    line-height:1.45;
  ...
  @media (max-width:390px) {
    #settingsModal .settings-section:not(.hidden-element) { padding:14px; border-radius:16px; }
  }
</style>`;
...
(() => {
  ...
  function setPermissionStatus(status) {
    ...
    if (permissionStatus === 'granted') {
      ...
      label.textContent = native ? 'Ask next time' : 'Not yet requested';
    } else {
      label.textContent = native ? 'Checking…' : 'Check browser settings';
    }
  }

  function markFromGeolocationError(error) {
    if (Number(error?.code) === 1) setPermissionStatus('denied');
  }
  ...
  function wrapWebGeolocation() {
    ...
    if (originalWatch) {
      geo.watchPosition = (success, error, options) => originalWatch(
        position => { setPermissionStatus('granted'); success?.(position); },
        err => { markFromGeolocationError(err); error?.(err); },
        options
      );
    }
    if (originalGet) {
      geo.getCurrentPosition = (success, error, options) => originalGet(
        position => { setPermissionStatus('granted'); success?.(position); },
        err => { markFromGeolocationError(err); error?.(err); },
        options
      );
  ...
  async function refreshPermissionStatus() {
    if (window.__SPEEDO_NATIVE_IOS__) {
      try {
        const status = await window.__SPEEDO_NATIVE_PERMISSIONS__?.refresh?.();
        setPermissionStatus(status || 'unknown');
      } catch (_) { setPermissionStatus('unknown'); }
      return;
    }
    if (!navigator.permissions?.query) {
      setPermissionStatus(permissionStatus === 'granted' ? 'granted' : 'unknown');
      return;
    }
    try {
  ...
  function installLocationSetting() {
    ...
    setting.innerHTML = `
      <div class="location-permission-row">
        <div>
          <div class="setting-title" style="font-size:19px;">Location access</div>
          <div class="location-permission-status" id="locationPermissionStatus"><span class="location-permission-dot"></span><span id="locationPermissionLabel">Checking…</span></div>
        </div>
        <button class="location-permission-action" id="locationPermissionAction" type="button">${native ? 'Manage' : 'How to change'}</button>
      </div>
      <div class="location-permission-help" id="locationPermissionHelp">${native ? 'Location permission is controlled by iPhone Settings.' : locationHelpText()}</div>`;
    body.prepend(setting);

    document.getElementById('locationPermissionAction')?.addEventListener('click', async () => {
      if (native) {
        await window.__SPEEDO_NATIVE_PERMISSIONS__?.openSettings?.();
        return;
      }
  ...
  function polishSettings() {
    ...
    modal.querySelectorAll('.setting-title').forEach(title => {
      if (title.textContent.trim() === 'Need help?') title.textContent = 'Feedback?';
    });
    ...
    if (!footer && sheet) {
      ...
      footer.innerHTML = `<span class="version">${window.__SPEEDO_NATIVE_IOS__ ? `Version 1.0 · Build ${APP_VERSION}` : `Version ${APP_VERSION}`}</span>Frenano · Made in Switzerland`;
  ...
  wrapWebGeolocation();
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', polishSettings, { once:true });
  else polishSettings();
