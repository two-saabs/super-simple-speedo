...
<style id="settings-redesign-v1">
  ...
  const js = `\n<script id="settings-redesign-v1-script">\n(() => {\n  const decorateSettings = () => {\n    const modal=document.getElementById('settingsModal'); if(!modal||modal.dataset.redesigned==='true')return; modal.dataset.redesigned='true';\n    const icons={audio:'♪',display:'☼',statistics:'▥',about:'ⓘ',privacy:'◇','advanced-and-experimental-features':'⚗'}; const labels={display:'Appearance & Display',statistics:'Usage & Statistics'};\n    modal.querySelectorAll('.settings-section').forEach(section=>{const key=section.dataset.settingsSection||'';const header=section.querySelector('.settings-section-header');const title=section.querySelector('.settings-section-title');if(header)header.dataset.settingsIcon=icons[key]||'•';if(title&&labels[key])title.textContent=labels[key];});\n    const sheet=modal.querySelector('.sheet');if(sheet&&!sheet.querySelector('.settings-redesign-note')){const note=document.createElement('div');note.className='settings-redesign-note';note.textContent='Super Simple Speedo · Made in Switzerland';sheet.appendChild(note);}\n  };\n  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',decorateSettings,{once:true});else decorateSettings();\n})();\n</script>`;
