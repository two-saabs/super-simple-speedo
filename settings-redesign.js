...
<style id="settings-redesign-v1">
  ...
  const replacements = [
    ...
    ['        state.lastAcceptedSpeed = candidateKmh;','        state.lastAcceptedSpeed = candidateKmh;\n        if (state.statisticsEnabled && candidateKmh > (state.stats.maxSpeed || 0)) { state.stats.maxSpeed = Math.round(candidateKmh); saveStats(); }'],
    ['          state.acceptedAutoRoad = displayRoadName(roadName);','          state.acceptedAutoRoad = displayRoadName(roadName);\n          if (state.statisticsEnabled && state.acceptedAutoRoad && previousConfirmedRoad !== state.acceptedAutoRoad) { state.stats.roadsIdentified = (state.stats.roadsIdentified || 0) + 1; saveStats(); }'],
  ...
  for (const [before, after] of replacements) { if (!html.includes(before)) throw new Error(`Settings statistics polish: expected snippet not found: ${before.slice(0,80)}`); html = html.replace(before, after); }
  const js = `\n<script id="settings-redesign-v1-script">\n(() => { const decorateSettings=()=>{const modal=document.getElementById('settingsModal');if(!modal||modal.dataset.redesigned==='true')return;modal.dataset.redesigned='true';const icons={audio:'♪',display:'☼',statistics:'▥',about:'ⓘ',privacy:'◇','advanced-and-experimental-features':'⚗'},labels={display:'Appearance & Display',statistics:'Usage & Statistics'};modal.querySelectorAll('.settings-section').forEach(section=>{const key=section.dataset.settingsSection||'',header=section.querySelector('.settings-section-header'),title=section.querySelector('.settings-section-title');if(header)header.dataset.settingsIcon=icons[key]||'•';if(title&&labels[key])title.textContent=labels[key];});const sheet=modal.querySelector('.sheet');if(sheet&&!sheet.querySelector('.settings-redesign-note')){const note=document.createElement('div');note.className='settings-redesign-note';note.textContent='Super Simple Speedo · Made in Switzerland';sheet.appendChild(note);}};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',decorateSettings,{once:true});else decorateSettings();})();\n</script>`;
  if (!html.includes('</head>') || !html.includes('</body>')) throw new Error('Settings redesign: expected HTML closing tags not found');
  html=html.replace('</head>',`${css}\n</head>`); html=html.replace('</body>',`${js}\n</body>`); return html;
}
