...
const contracts = [
  ...
  ['journey mode remains experimental', /id=["']experimentalJourneySetting["'][^>]*aria-hidden=["']true["']/],
  ['journey mode has a user toggle', /id=["']journeyModeSwitch["']/],
  ['journey mode reuses transport detective', /state\.journeyModeEnabled[\s\S]*state\.transportDetectiveEnabled/],
  ['journey timeline is session-only', /journeySegments:\s*\[\]/],
  ['journey records walking and public transport labels', /journeyModeName[\s\S]*walking:[\s\S]*bus:[\s\S]*tram:[\s\S]*train:/],
  ['journey waits before creating a waiting segment', /JOURNEY_WAITING_SEGMENT_DELAY_MS\s*=\s*90\s*\*\s*1000/],
  ['journey UI replaces duplicate transport badge', /showDetectorBadge\s*=\s*available\s*&&\s*standaloneEnabled/],
  ['journey is presented as recommended all-in-one mode', /Recommended · all-in-one/],
  ['transport detective is presented as standalone mode', /experimental-choice-kicker standalone[^>]*>Standalone</],
  ['standalone detector switch is visually exclusive from journey', /standaloneEnabled\s*=\s*state\.transportDetectiveEnabled\s*&&\s*!state\.journeyModeEnabled/],
  ['journey switch controls its transport engine dependency', /state\.journeyModeEnabled\s*=\s*turningOn;[\s\S]*state\.transportDetectiveEnabled\s*=\s*turningOn/],
  ['choosing standalone transport exits journey mode', /transportDetectiveSwitch[\s\S]*state\.journeyModeEnabled\s*=\s*false/],
  ...
  ['v13.4.1 stationary ground truth exists', /data-ground-truth=["']stationary["'][^>]*>I’m not moving<\/button>/],
  ['v13.4.1 launch disclaimer is addressable', /id=["']launchDisclaimer["']/],
  ['v13.4.1 launch disclaimer hides after Lets go', /beginDriveFromLaunch[\s\S]*launchDisclaimer[\s\S]*classList\.add\(["']hidden["']\)/],
  ['v13.4.1 footer disclaimer stays hidden', /id=["']driverResponsibility["'][^>]*class=["']hidden["'][^>]*aria-hidden=["']true["']/],
