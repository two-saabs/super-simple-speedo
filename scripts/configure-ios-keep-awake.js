...
function configureKeepAwake(swift) {
  if (!swift.includes(launchMarker)) {
    ...
    if (!match) throw new Error("didFinishLaunchingWithOptions could not be found in AppDelegate.swift");
  ...
  if (legacyGeneratedActiveMethod.test(swift)) swift = swift.replace(legacyGeneratedActiveMethod, "");
  ...
  if (!swift.includes(activeMarker)) {
    ...
    if (existingActive) {
      ...
      if (classEnd === -1) throw new Error("AppDelegate class ending could not be found");
...
function configureFile(appDelegatePath = defaultAppDelegatePath) {
  if (!fs.existsSync(appDelegatePath)) throw new Error(`${appDelegatePath} was not found. Run \`npx cap add ios\` first.`);
