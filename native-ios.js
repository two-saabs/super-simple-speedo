...
async function configureNativeStatusBar() {
  try {
    ...
    console.warn('[FRENANO_STATUSBAR] configuration failed', String(error?.message || error));
  }
}

function permissionError(message = 'Location permission denied.') { return { code: 1, message }; }
...
async function currentPermission() {
  try { const result = await Geolocation.checkPermissions(); return result.location || 'unknown'; }
...
async function clearNativeWatch(record) {
  ...
  try { await Geolocation.clearWatch({ id: record.nativeId }); } catch (_) {}
}
...
function logLayoutDiagnostic(reason) {
  ...
  console.log('[FRENANO_LAYOUT]', JSON.stringify({
    reason,
    innerWidth:window.innerWidth, innerHeight:window.innerHeight,
    clientWidth:document.documentElement.clientWidth, clientHeight:document.documentElement.clientHeight,
    visualViewport:vv ? { width:Math.round(vv.width), height:Math.round(vv.height), offsetTop:Math.round(vv.offsetTop), offsetLeft:Math.round(vv.offsetLeft), scale:vv.scale } : null,
    orientation:orientation ? { type:orientation.type, angle:orientation.angle } : null,
    app:rectFor('#app'), main:rectFor('main'), cluster:rectFor('.cluster'), settings:rectFor('#settingsButton')
  }));
}
  ...
  watchPosition(success, error, options = {}) {
    ...
    (async () => {
      try {
        ...
        if (status !== 'granted') { watches.delete(localId); error?.(permissionError()); return; }
        const nativeId = await Geolocation.watchPosition({
          ...
          if (pluginError) { error?.(normaliseError(pluginError)); return; }
          if (position) success?.(position);
        });
        record.nativeId = nativeId;
        if (record.cancelled) { await clearNativeWatch(record); watches.delete(localId); }
      } catch (nativeError) { watches.delete(localId); error?.(normaliseError(nativeError)); }
    })();
    return localId;
  },
  clearWatch(localId) {
...
window.__SPEEDO_NATIVE_PERMISSIONS__ = {
  ...
  async openSettings() {
    try { await AppLauncher.openUrl({ url:'app-settings:' }); }
    catch (_) { window.location.href = 'app-settings:'; }
  }
};
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') refreshPermissionUi(); });
